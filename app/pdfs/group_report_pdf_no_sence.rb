class GroupReportPdfNoSence < Prawn::Document
	include ReportsHelper

	def initialize(institution, group_name, filter, date, view_context)
		super(:margin => 50)
		font "Helvetica"
		...
		bounding_box([bounds.left, bounds.top - 40], :width  => bounds.width, :height => bounds.height - 80) do
			...
			general_data(institution, group_name, date, filter)
			course_attendance(members, filter, date)
			indicadores_academicos(members, filter, date)
		end	
	...
	def general_data(institution, group_name, date, filter)
		if filter.to_i == 1
			group = "Curso"
		else
			group = "Departamento"
	...
	def course_attendance(members,filter,date)
		move_down 20
		font "Helvetica", :style => :bold
		...
		data = []
		data << ["<b>Nombre</b>", "<b>"+table_group+"</b>", "<b>Presente</b>", "<b>Ausente / Tarde</b>", "<b>Clases Realizadas</b>", "<b>Asistencia<b>"]	#encabezado de la tabla		
		members.each do |member|
			...
			data << [member_data.firstname+" "+member_data.lastname, member_group, member_data.p_sessions.to_s, a_sessions.to_s, member_data.current_sessions.to_s+" de "+member_data.total_sessions.to_s, "<b>"+att_pct.to_s+"%</b>"]
		end
	...
	def indicadores_academicos(members, filter, date)
		move_down 15
		font "Helvetica", :style => :bold
