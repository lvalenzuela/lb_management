class GroupReportPdf < Prawn::Document
	...
	def find_group_members(groupid)
		last_report_date = CourseGroupReport.all().order("created_at DESC").first().created_at
		return members = User.joins("inner join course_group_reports as cgr
									on cgr.userid = cgr.userid
									and cgr.groupid = #{groupid}
									and cgr.created_at = #{last_report_date}")
	end
end